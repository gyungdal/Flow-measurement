...
bool Test::eeprom(){
    ...
    eeprom_list_t* generate = this->generate_eeprom_list();
    ...
    eeprom_list_t* eepromData = get_eeprom_data_list();
    ...
    Serial.print("[MATCH] ");
    Serial.println(memcmp(generate->items, eepromData->items, (sizeof(eeprom_item_t) * generate->length)));
    
    if(memcmp(generate->items, eepromData->items, (sizeof(eeprom_item_t) * generate->length)) != 0){
...
eeprom_list_t* Test::generate_eeprom_list(){
